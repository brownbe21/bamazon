...
connection.connect(function(err) {
    ...
    new Promise(function(resolve, reject) {
        connection.query('SELECT * FROM products', function(err, res) {
            if (err) reject(err);
            resolve(res);
            console.log('This is Bamazon: Please choose from our Products')
        });
        // Console log each item and increment the number of products
    }).then(function(result) {
        result.forEach(function(item) {
            numberOfProductTypes++;
            console.log('Item ID: ' + item.item_id + ' || Product Name: ' + item.product_name + ' || Price: ' + item.price + ' || Department Name: ' + item.department_name + ' || Stock Quantity: ' + item.stock_quantity);
        });
        // Enter the store
    }).then(function() {
        return enterStore();
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (savedData.answer) {
                ...
                connection.query('UPDATE products SET ? WHERE ?', [{
                    ...
                    if (err) throw err;
                    console.log('Your order total cost $' + totalCost + '. Your order is now complete, thank you');
                    connection.destroy();
                });
            } else {
